...
START_COLOR =  (135, 206, 235) # Blue (for ghost)
GOAL_COLOR = (255, 255, 0)  # Yellow (for pacman)
EXPLORED_COLOR = (255, 165, 0)  # Orange (for explored nodes)
PATH_COLOR = (173, 216, 230) # Light blue (for path) 
...
class BlueGhostBFS():
    ...
    def solve(self):
        ...
        frontier_set = {self.start}  # Set to track states in the frontier
        ...
        while frontier:
            ...
            if current == self.goal:
                ...
                return {
                    ...
                    'cost': len(cells)  # BFS does not use cost, so path length is used
                }
            ...
            for action, next_state in self.neighbors(current):
                if next_state not in self.explored and next_state not in frontier_set:
    ...
    def visualize_pygame(self):
        ...
        pygame.display.set_caption("Blue Ghost BFS Visualization")
        ...
        while running:
            for event in pygame.event.get():
                if event.type == pygame.QUIT:
                    ...
                    if event.key == pygame.K_RIGHT:
                        ...
                        filename = f"blue_ghost_step_{current_step_idx}.png"
            ...
            for i_row in range(self.height):
                for j_col in range(self.width):
                    ...
                    if self.walls[i_row][j_col]:
                        ...
                        fill = (255, 100, 0)  # Bright blue for newly added
                    elif (i_row, j_col) == step['current']:
                        fill = (255, 80, 0)  # Very bright blue for current
                    else:
                        fill = EMPTY_COLOR
            ...
            # Title
            title_text = "Blue Ghost BFS Algorithm Visualization"
...
if __name__ == "__main__":
    if len(sys.argv) != 2:
        print("Usage: python blue_ghost_visualization.py maze.txt")
        sys.exit(1)

    # Initialize and solve
    ghost = BlueGhostBFS(sys.argv[1])
    print("Solving with Blue Ghost BFS algorithm...")
    result = ghost.solve()
    ...
    if result:
        ...
        # Generate images
        ghost.output_image(f"blue_ghost_{ghost.goal[0]}_{ghost.goal[1]}.png", show_solution=True, show_explored=False)
        ghost.output_image(f"blue_ghost_explored_{ghost.goal[0]}_{ghost.goal[1]}.png", show_solution=True, show_explored=True)
